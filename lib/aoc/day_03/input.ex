defmodule AOC.Day03.Input do
  ...
  def parsed_input do
    read_wire_instructions(@input)
  end

  def read_wire_instructions(string) do
    string
    |> String.split("\n", trim: true)
    |> Enum.map(fn(line) ->
      read_wire_directions(line)
    end)
  end

  defp read_wire_directions(string) do
    string
    |> String.split(",", trim: true)
    |> Enum.map(fn(dir) ->
      read_individual_direction(dir)
    end)
  end

  defp read_individual_direction(string) do
    string
    |> String.trim()
    |> read_direction()
  end

  defp read_direction(<<"R"::utf8, rest::binary>>) do
    {r, _} = Integer.parse(rest)
    {r,0}
  end

  defp read_direction(<<"L"::utf8, rest::binary>>) do
    {r, _} = Integer.parse(rest)
    {-r,0}
  end

  defp read_direction(<<"U"::utf8, rest::binary>>) do
    {u, _} = Integer.parse(rest)
    {0,u}
  end

  defp read_direction(<<"D"::utf8, rest::binary>>) do
    {u, _} = Integer.parse(rest)
    {0,-u}
  end
end
